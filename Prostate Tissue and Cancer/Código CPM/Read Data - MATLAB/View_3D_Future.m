...
type = 'Future_Geometry';
run_num = '3';
sim_time = '1000';

fileID = fopen(strcat("Simulation_Data/",type,"/Future_Test_TumorDynamics",run_num,".dat"),'r');
fileEVO = fopen(strcat("Simulation_Data/",type,"/Future_Cell_Number_EVO_",run_num,".txt"),'r');
...
xlabel('X')
ylabel('Y')
% [xx,yy,zz] = meshgrid(1:Ny,1:Nx,1:Nz);
% s = isosurface(xx, yy, zz, C0, 1);
% p = patch(s);
...
saveas(gcf,"Simulation Graphs Results\"+type+"\SlidesView"+run_num,'pdf')

figure(2)
saveas(gcf,"Simulation Graphs Results\"+type+"\TumorView"+run_num,'pdf')