...
int main(int argc, char* argv[]) {
	...
	// Future type 3D cells geometry 
	Nx = 500; Ny = 250; Nz = 40;
	double vtarget1 = 6548220;
	double vtarget2 = 795; 
	double vtarget3 = 340; 
	double vtarget4 = 6354180; 
	double vtarget5 = 795;		
	double vtarget6 = 10000;			
	fs.open("Prostate3DCellsFuture.txt");
	std::string geom = "Future";
	...
	double J01 [7] = {1.8000,    Jxx,    J11,    J11,    J11,    J11,	J11}; 		// Lumen adhesion matrix
	double J02 [7] = {J01[0], J01[2],    Jxx,    J11,    J11,    J11,	J11}; 		// Luminal cells adhesion matrix
	double J03 [7] = {J01[0], J01[3], J02[3],    Jxx,    J11,    J11,	J11};		// Basel cells adhseion matrix
	double J04 [7] = {J01[0], J01[4], J02[4], J03[4],    Jxx,    J11,	J11}; 		// Stroma adhesion matrix
	double J05 [7] = {J01[0], J01[5], J02[5], J03[5], J04[5], 	 Jxx,	J11}; 		// Tumoral cells adhesion matrix
	double J06 [7] = {J01[0], J01[6], J02[6], J03[6], J04[6], J05[6],	Jxx}; 		// Connection Lumen adhesion matrix
	

	// Hamiltonian Parameters - Cell characteristics and Parameters Weight
	// Target volume deviation parameters
	double lambdav1 = 1700;  	// Lumen
	double lambdav2 = 1900;		// Luminal cells
	double lambdav3 = 2300;		// Basal cells
	double lambdav4 = 1e12;		// Stroma
	double lambdav5 = 8800;		// Tumoral cells
	double lambdav6 = 1700; 	// Connection Lumen
	...
	double ltarget6 = 1;	// Connection Lumen
	...
	bool z_periodic = false;				// z axis periodic condition 
	...
	log_file << "Proliferation Time: 25\n";
	log_file.close();
	...
	cpm.add_type(lambdav6, vtarget6, lambdal, ltarget6, 0, &J05[0], 0); 	// Tumoral cells
	...
	float ti, tf, pi, pf, zi, zf;							// Create the float variables to store the cell angle variations 
  	...
  	while (getline(fs, Linha)) {			// Reads the file with the cell geometry line by line and stores each line in the Linha Variable
		...
		if (geom.compare("SecondGeom") == 0) {
			if ( (tumor_layer==2) && (xcell==620)) xtype = 5; 
			if ( (tumor_layer==3) && (xcell==1820)) xtype = 5;
		}

		if (geom.compare("SecondGeomLarge") == 0) {
			if ( (tumor_layer==2) && (xcell==1220)) xtype = 5; 
			if ( (tumor_layer==3) && (xcell==3740)) xtype = 5;
		}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if( nchanged>0 ) std::cout << " nchanged = " << nchanged << std::endl;
		...
		if (i%1==0) { 
			...
			NewFile << i << " " << n_cell[0] << " " << n_cell[1] << " " << n_cell[2] << " " << tumor_volume << " " << tumor_area << std::endl;
		}
		...
		if (i%50==0) {
        	...
        	std::cout << "Pixel_Copys = " << elapsed1.count() << "; Cell_Proliferation = " << elapsed2.count() << "; Tumor_Area = " << elapsed3.count() << "; Cells_Death = " << elapsed4.count() << ";\n" << std::endl;
