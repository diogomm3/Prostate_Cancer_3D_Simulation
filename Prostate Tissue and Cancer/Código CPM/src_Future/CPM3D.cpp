...
bool CPM::add_cell_vol(float pi, float pf, float ti, float tf, float zi, float zf, int newtau, int cellnum, int Ny) { 		// Function to add the cells within the given area
	...
	int xi,xf,yi,yf,rot_xi,rot_xf,rot_yi,rot_yf,rot_zi,rot_zf;
	// std::cout << pi << "\t" << pf << "\t" << ti << "\t" << tf << "\t" << zi << "\t" << zf << "\t" << std::endl;
	...
	if (cellnum > 1604) {
		...
		rot_zi = Nz/2 - (xf-xi);
		rot_zf = Nz/2 + (xf-xi);
		std::cout << rot_xi << "\t" << rot_xf << "\t" << rot_yi << "\t" << rot_yf << "\t" << rot_zi << "\t" << rot_zf << "\t" << std::endl;
	}
			...
			for (int ix=0; ix<Nx; ix++) {
				for (int iy=0; iy<Ny; iy++) {
					for (int iz=0; iz<Nz; iz++) {
							...
							// 	}
							// }
							// else if (cellnum <= 1604) {
							// 		if (ix >= 250) {
							// 			rho = sqrt((ix-(xmed+250))*(ix-(xmed+250)) + (iy-ymed)*(iy-ymed));
							// 			phi = atan2(iy-ymed, ix-(xmed+250));
							// 			// if (ix == 250 && iy == 125) {
							// 			// 	std::cout << rho << "\n";
							// 			// }
							// 			if (phi < 0) {phi += 2*PI;}
							...
							// }
							if (cellnum > 1604) {
								if (ix >= rot_xi && ix <= rot_xf && iy >= rot_yi && iy <= rot_yf && iz >= rot_zi && iz <= rot_zf) {
									indexy = ix*Ny*Nz + iy*Nz + iz;
									PIXELS[indexy].tag = &(c_list->CELL);
									aux->CELL.vol++;
									aux->CELL.voxels.push_back(indexy);
...
// int CPM::tumor_area () {		// calculate tumor superficial area
// 	int pixel_index;
// 	int tumor_area = 0;
...
// 				pixel_index = i*Ny*Nz + j*Nz + k;
// 				if (PIXELS[pixel_index].tag->tau->index == 5) {
// 					int neighbours_index_not5 = 0;
// 					if (i == 0) {
// 						int neighbour_index1 = (i+1)*Ny*Nz + j*Nz + k;
// 						if (PIXELS[neighbour_index1].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}	
// 					}
// 					else if (i == Nx-1) {
// 						int neighbour_index2 = (i-1)*Ny*Nz + j*Nz + k;
// 						if (PIXELS[neighbour_index2].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}
// 					}
// 					else {
// 						int neighbour_index1 = (i+1)*Ny*Nz + j*Nz + k;
// 						int neighbour_index2 = (i-1)*Ny*Nz + j*Nz + k;
// 						if (PIXELS[neighbour_index1].tag->tau->index != 5 || PIXELS[neighbour_index2].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}
// 					}
// 					if (j == 0) {
// 						int neighbour_index3 = i*Ny*Nz + (j+1)*Nz + k;
// 						if (PIXELS[neighbour_index3].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}
// 					}
// 					else if (j == Ny-1) {
// 						int neighbour_index4 = i*Ny*Nz + (j-1)*Nz + k;
// 						if (PIXELS[neighbour_index4].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}	
// 					}
// 					else {
// 						int neighbour_index3 = i*Ny*Nz + (j+1)*Nz + k;
// 						int neighbour_index4 = i*Ny*Nz + (j-1)*Nz + k;
// 						if (PIXELS[neighbour_index3].tag->tau->index != 5 || PIXELS[neighbour_index4].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}

// 					}
// 					if (k == 0) {
// 						int neighbour_index5 = i*Ny*Nz + j*Nz + (k+1);
// 						if (PIXELS[neighbour_index5].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}
// 					}
// 					else if (k == Nz-1) {
// 						int neighbour_index6 = i*Ny*Nz + j*Nz + (k-1);
// 						if (PIXELS[neighbour_index6].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}
// 					}
// 					else {
// 						int neighbour_index5 = i*Ny*Nz + j*Nz + (k+1);
// 						int neighbour_index6 = i*Ny*Nz + j*Nz + (k-1);
// 						if (PIXELS[neighbour_index5].tag->tau->index != 5 || PIXELS[neighbour_index6].tag->tau->index != 5) {
// 							neighbours_index_not5++;
// 						}
// 					}
// 					if (neighbours_index_not5 != 5) {
// 						tumor_area++;
// 					}
// 				}
// 			}
...
double CPM::dH_vol(cell* tcell, cell* scell) const {	// change of energy due to change of cell volume
	...
	if (scell != nullcell) {
		...
		if (types==5) { // tumor cell with dynamic target volume
			...
			V_T = VT + 500*(scell->timeProlif); // linear growth
	...
	if (tcell != nullcell) {
		...
		if (type==5) { // tumor cell with dynamic target volume
			...
			V_T = VT + 500*(tcell->timeProlif); // linear growth
			if (V_T>VT*2) V_T = VT*2; // volume limit
			// std::cout << "V_T: " << V_T << std::endl;
		}
