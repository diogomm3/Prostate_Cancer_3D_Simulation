...
void CPM::setup(int nx, int ny, int nz, int seed) { // setup of CPM			// Defines the setup function builted in the CPM class
	...
	h_periodic = true;  // false  border conditions
	v_periodic = false;	// y
	d_periodic = true;	// z
...
void CPM::free_mem(void) {  // free memory to avoid problems...
...
void CPM::init() { // initialization of CPM
	...
	t_list = new l_type; // for each cell type
	t_list->next = NULL;
	t_list->TYPE.index = 0;
...
// add new cell type, with all the necessary parameters
	...
	prev->next = newtype;
	delete [] Jaux;
	std::cout << " add_type n_types " << n_types << " V_target " << V_target << " L_target " << L_target << std::endl;
...
bool CPM::add_cell(int index, int newtau) {  // add new cell (with all the parameters)
...
bool CPM::add_cell_vol(int xi, int xf, int yi, int yf, int zi, int zf, int newtau, int Ny) { // add a cell within area within given limits
	...
	if (newtau>0 && newtau < n_types) {
		...
		int indexy = xi*Ny*Nz + yi*Nz + zi;
		////int indexy1, flag, jup, jdo, ile, iri, kfr, kba, i, j, k;

		a = PIXELS[indexy].tag == nullcell;
		b = PIXELS[indexy].tag->vol > 1;
		...
		if (a||b) {
			...
			for (int ix=xi; ix<=xf; ix++) {
				for (int iy=yi; iy<=yf; iy++) {
			...
			std::cout << aux->CELL.tau->index <<" vol " << aux->CELL.vol << " size " << aux->CELL.voxels.size() << std::endl;
			/*
			for (int iv=0; iv<aux->CELL.voxels.size(); iv++) {
				indexy = aux->CELL.voxels[iv];
				...
				for (int ii = 0; ii<6; ii++) {
					for (int iv1=0; iv1<aux->CELL.voxels.size(); iv1++) {
						indexy1 = aux->CELL.voxels[iv1];
						if (index6[ii]==indexy1) flag++;
					}
				}
				if (flag<6) index_border.push_back(indexy); // at least one voxel not from the same cell
				//std::cout << " iv " << iv << " flag " << flag << std::endl;
			}
			std::cout << " index_border " << index_border.size() << std::endl;
			*/

			//std::cout << " vol " << aux->CELL.vol << " size " << aux->CELL.voxels.size() << std::endl;
...
int CPM::proliferation(double vol_mitosis) { // cell proliferation
	...
	while (current_node) {
		...
		if ( type == 8 ) {  // tumor cell
			...
			double timeLimit = 24 + 3*sqrt(-2*log(u1))*cos(2*PI*u2);  // gaussian mu=24, sigma=3
			...
			if ( (volume > 1.50*V_target) && ((current_cell->timeProlif) > timeLimit) ) {  //
/*				flag_ur = check_contact (current_cell);	//  in contact with urine?
				prob = ((int)(genrand_int31() % 100));
				if ( (flag_ur==0) || ((flag_ur==1)&&(prob<1)) ) {   // papillary?
				//if ( (flag_ur==1) || ((flag_ur==0)&&(prob<1)) ) {   // CIS?
					flag = mitosis (current_cell);	//  5
					//std::cout << " proliferation type = " << type << std::endl;
				}
*/
/*
				int ytumor = -999;
...
void CPM::step(int index_t, int nbr) { // pixel copy attempt
	...
	if (check) {
		...
		dH = dH_vol(tcell, scell) + dH_adh(target, scell) + dH_area(scell, tcell, index_t);  // change in the Hamiltonian (with different contributions)
		...
		if (check) {
			...
			//take_step(target, scell, lent, lens);
                        if (check2) { take_step(target, scell, tcell, index_t, lent, lens);
...
void CPM::rand_step() {  // take a random pixel copy attempt
	...
	int index_t = ((int)(genrand_int31() % NE));
	////int indexy = ((int)(genrand_int31() % (index_border.size()) ));
	////int index_t = index_border[indexy];

	//std::cout << " index_t " << index_t << std::endl;
	int nbr;
	int a = 1, b = 1, c = 1;
	///if (!h_periodic) a = (index_t/Ny)%(Nx-1);
	///if (!v_periodic) b = (index_t%Ny)%(Ny-1);
	...
	if (tcell!=scell) { // source and target are from different cells
		...
		if (tcell != nullcell) { // and not the medium
			...
			if ((tcell->vol) > 1) { // and not disapear
				...
				if ( (n==1) || (typet==1) || (typet==7) ) { return true; } // no connectivity in urine or stroma if only a pixel don't need to test connectivity!
...
double CPM::dH_vol(cell* tcell, cell* scell) const {	// change of energy due to change of cell volume
	...
	if (scell != nullcell) {
		...
		types = scell->tau->index;
		if (types==8) { // tumor cell with dynamic target volume
			V_T = 125 + 5*(scell->timeProlif); // linear growth
			if (V_T>250) V_T = 250; // volume limit
			///l = l*(V_T/25)*0.80;
			//V_T = 25;
			//for (int imcs=0; imcs < scell->timeProlif; imcs++) V_T = k*Vmax + V_T*(1-k); // iterative, saturation
			//std::cout << " Tum V = " << vol << " V_T " << V_T << std::endl;
		}
	...
	if (tcell != nullcell) {
		...
		if (type==8) { // tumor cell with dynamic target volume
			V_T = 125 + 5*(tcell->timeProlif); // linear growth
			if (V_T>250) V_T = 250; // volume limit
			///l = l*(V_T/25)*0.80;
			//V_T = 25;
			//for (int imcs=0; imcs < tcell->timeProlif; imcs++) V_T = k*Vmax + V_T*(1-k); // iterative, saturation
		}

		dh += l*(1-2*(vol-V_T))/(V_T*V_T); // Hamiltonian change
		//if ((type==7)&&(types==8)) std::cout << " Uri dh = " << dh << " V = " << vol << " V_T " << V_T << std::endl;
	}

	return dh;
}

double CPM::dH_vol2(cell* tcell, cell* scell) const {	// change of energy due to change of cell volume
	...
	if (scell != nullcell) {
		...
		dh += l*(1+2*(vol-V_T)); // Hamiltonian change
	}
...
double CPM::dH_len(cell* tcell, cell* scell, int index_t, double* lent, double* lens) const {   // cell length. needs to be optimized! and checked...
        ...
        std::set<int> mysetT, mysetS;

        mysetT = tcell->cell_content;
        mysetS = scell->cell_content;
	...
	if (scell != nullcell) { // not medium
		...
		if (l>0) {
	        for (it1=mysetS.begin(); it1!=mysetS.end(); ++it1) { // scan all pixels
      			int i = *it1/Ny;
        		int j = *it1%Ny;
			///lT = sqrt( (i-it)*(i-it) + (j-jt)*(j-jt) );
			lT = sqrt( (fabs(i-it)+1)*(fabs(i-it)+1) + (fabs(j-jt)+1)*(fabs(j-jt)+1) );
			if (lT>len1) len1 = lT;
        	}
		if (len1<len) {
			len1 = len;
	...
	if (tcell != nullcell) { // not medium
		...
		if (l>0) {
	        for (it1=mysetT.begin(); it1!=mysetT.end(); ++it1) {

                	if( *it1 != index_t ) {
	      			int i1 = *it1/Ny;
        			int j1 = *it1%Ny;

			        for (it2=it1; it2!=mysetT.end(); ++it2) {

                			if( (*it2 != index_t) && (*it2 != *it1) ) {
	      					int i2 = *it2/Ny;
  				      		int j2 = *it2%Ny;

						lT = sqrt( (fabs(i2-i1)+1)*(fabs(i2-i1)+1) + (fabs(j2-j1)+1)*(fabs(j2-j1)+1) );
						if (lT>len1) len1 = lT;
        				}
                        	}
                	}
...
int CPM::n_cell() { // number of cells
        ...
        while (current_node) {
                ...
                volume = current_cell->vol;
                type = current_cell->tau->index;
		///if ( (volume>vol_max) && (type==1) ) vol_max = volume;
		////if ( (volume>vol_max) && ( (type==1) || (type==2) ) ) vol_max = volume;
...
void CPM::death_pressure()  { // removal of small cells (due to tumor pressure)
	...
	double volume, volumeT;
		...
		type = current_cell->tau->index;
                ...
                if( (type==3) || (type==5) || (type==6) ) { // only basal, intermediate or umbrella
                        volume = current_cell->vol;
